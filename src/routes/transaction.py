...
def create_financial_record(data: TransactionCreate, session: SessionDep, user = Depends(require_roles([UserRole.admin, UserRole.analyst]))):
...
def update_financial_record(record_id: int, data: TransactionUpdate, session: SessionDep, user = Depends(require_roles([UserRole.admin, UserRole.analyst]))):
    record = get_record_by_id(session, record_id)
...
def delete_financial_record(record_id: int, session: SessionDep, user = Depends(require_roles([UserRole.admin, UserRole.analyst]))):
    record = get_record_by_id(session, record_id)
